 ...
 * 2、思想很简单，就是在一个有序的链表上面随机加索引，
 *      规定每个节点多可以有k个向后的不同高度的链接，高层的索引一次跨越的数据更多，
 *   第0层的forward指针就是普通单链表的next指针，如果k=0，那么就是普通的单链表了。
 *   1）查找：每次遍历的时候，从表头开始，从最高层索引开始比较，因为NIL的key值
 *              是最大值，所以可以每次比较key值，小于给定的key则继续往后，
 *              否则，向当前节点的下一层k进行比较。直到k=0，此时后一个节点就是
 *              找到的节点，再判断key是否等于给定的key。
 *   2）插入：先按照查找的方法找到插入位置，在查找的时候要同时记录，
 *              每一个查找时的拐点，记录在update[MAX_NUMBER_OF_LEVEL]中，
 *              直到0，然后随机生成一个要插入的节点及其节点的层数tmp_t，
 *              如果层数tmp_t > 最高层level，则level++，当前节点设为最高层数，
 *              同时把update[level]填入表头。update记录的是插入节点后，
 *              可能会被该节点挡到的节点，也就是update[0-tmp_t]这些节点的对应层的
 *              forward都要指向要插入的节点。同时把要插入的节点的forward[0-tmp_t]
 *              都要改成update对应节点的forward指针。
 *   3）删除：类似与插入，也是在找要删除的节点时记录拐点，然后找到要删除的节点后，
 *              更新update对应层的指针为要删的节点的对应层的forward指针。
 * 3、注意：
 *       1）每一个节点并不知道自己有几层，除表头为MAX_LEVEL层，表尾为1层外,
 *            均不知道自己有多少个forward指针。
