...
//----------------------------------------------------  
rb_node_t* rb_search_parent(key_t key, rb_node_t* root, rb_node_t** save)  
{  
...
{  
    return rb_search_parent(key, root, NULL);  
}  
...
{  
    ...
    if (!(node = rb_search_parent(key, root, NULL)))  //调用rb_search_auxiliary查找要删除的结点  
    {  
    ...
    if (color == BLACK)  
    {  
        root = rb_erase_fixup(child, parent, root); //调用rb_erase_rebalance来恢复红黑树性质  
    }  
